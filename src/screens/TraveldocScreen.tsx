...
export default function TraveldocScreen({ isFocused = true }: { isFocused?: boolean }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <ScreenHeading compact title="TravelDoc" subtitle={t('traveldocSub')} icon="description" />
      ...
      {isFocused && loadError && !loading && (
        <View style={[styles.loadingWrap, { backgroundColor: colors.bg }]}>
          ...
          <ScreenAction label={t('uiRetry')} icon="refresh" onPress={handleReload} />
        </View>
      )}
      {isFocused && (
        <WebView
...
const styles = StyleSheet.create({
  ...
  loadingWrap: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', zIndex: 10, paddingHorizontal: SPACING.xl },
