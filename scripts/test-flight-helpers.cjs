...
assert(easyjetOverlapMode.isFlightEasyJet({ flight: { airline: { name: 'easyJet', code: { iata: 'U2' } } } }), 'user-selected easyJet flights should keep their airline handling');
assert(!easyjetOverlapMode.isFlightEasyJet({ flight: { airline: { name: 'Wizz Air', code: { iata: 'W6' } } } }), 'other airlines must not use easyJet handling');
