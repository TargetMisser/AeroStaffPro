...
assert(releaseWorkflow.includes(':app:assembleRelease'), 'release workflow must build the phone release APK');
assert(releaseWorkflow.includes('AeroStaffPro-${{ steps.meta.outputs.tag }}.apk'), 'release workflow must expose the deterministic phone APK asset');
...
assert(windowsReleaseWorkflow.includes(':app:assembleRelease'), 'Windows release workflow must build the phone release APK');
assert(windowsReleaseWorkflow.includes('AeroStaffPro-${{ steps.meta.outputs.tag }}.apk'), 'Windows release workflow must expose the deterministic phone APK asset');
...
assert(releaseScript.includes('./gradlew clean :app:assembleRelease'), 'scripts/release-apk.sh must build the phone release APK');
assert(releaseScript.includes('android/app/build/outputs/apk/release/app-release.apk'), 'scripts/release-apk.sh must verify the phone APK output');

console.log(`Release check passed for phone v${packageVersion} (versionCode ${versionCodeMatch[1]}).`);
